...
class ImportCampaign implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        collect($cutting_array)->chunk(1000)->each(function ($lines) use (&$prepareForTemplateTag, $cost, $campaign, $db_fields, $user, &$key) {
        ...
        collect($insertData)->chunk(5000)->each(function ($lines) use (&$prepareForTemplateTag, $cost, $campaign, $db_fields, $user, &$key) {
        ...
        try {
            collect($prepareForTemplateTag)->sortBy('id')->values()->chunk(3000)->each(function ($sendData) use ($user, $campaign) {
                foreach ($sendData as $data) {
                    while ($user->customer->overQuota()) {
                        $campaign->failed(sprintf("Campaign `%s` (%s) halted, customer exceeds sending limit", $campaign->name, $campaign->uid));
                        sleep(60);
                    }
                    ...
                    if (substr_count($status, 'Delivered') == 1) {
                        $campaign->user->customer->countUsage($data['cost']);
