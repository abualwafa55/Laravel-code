...
class Campaigns extends SendCampaignSMS
{
    ...
    public function singleProcess()
    {
        ...
        collect($cutting_array)->chunk(1000)->each(function ($lines) use (&$prepareForTemplateTag, $cost) {
            ...
            foreach ($lines as $line) {
                ...
                $message = self::renderSMS($this->message, $line);
        ...
        collect($insertData)->chunk(5000)->each(function ($lines) use (&$prepareForTemplateTag, $cost) {
            foreach ($lines as $line) {
        ...
        try {
            collect($prepareForTemplateTag)->sortBy('id')->values()->chunk(3000)->each(function ($sendData) {
                foreach ($sendData as $data) {
//                    while ($this->user->customer->overQuota()) {
//                        $this->failed(sprintf("Campaign `%s` (%s) halted, customer exceeds sending limit", $this->campaign_name, $this->uid));
//                        sleep(60);
//                    }
                    $status = null;
                    if ($this->sms_type == 'plain' || $this->sms_type == 'unicode') {
                    ...
                    if (substr_count($status, 'Delivered') == 1) {
                        $this->user->customer->countUsage($data['cost']);
