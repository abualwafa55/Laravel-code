<div class="row ml-1">
    <div class="col-12">
        ...
        {{--sms sending credit--}}
        <div class="">
            <span class="text-uppercase"> {{ __('locale.plans.sms_sending_credits') }}</span>
            <span class="pull-right text-primary">
                                                       {{ __('locale.labels.using_limits',[
        'used' => \App\Library\Tool::format_number(Auth::user()->customer->getSendingQuotaUsage()),
        'remaining' =>  (Auth::user()->customer->getSendingQuota() == -1) ? __('locale.labels.unlimited') : \App\Library\Tool::format_number(Auth::user()->customer->getSendingQuota())
        ])}}
                                                    </span>
        </div>
        ...
        @else
            <div class="progress progress-bar-primary progress-xl">
                <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuenow="20" aria-valuemin="20" aria-valuemax="100" style="width:{{ Auth::user()->customer->displaySendingQuotaUsage() }}">{{ Auth::user()->customer->displaySendingQuotaUsage() }}</div>
            </div>
        @endif

        {{--sms sending credit end here--}}
